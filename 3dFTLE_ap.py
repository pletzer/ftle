...
def compute_ftle(ds, time_index, T, nsteps=1, method='RK45', atol=1.e-8, rtol=1.e-8):
    ...
    # turn the 1d axes into 3d arrays
    ...
    # should nx * ny * nz, ie the total number of nodes in the grid
    n = len(xflat)

    # read the data from file at the specified time index. Note the data are assumed to be 
    # face centred (Arakawa C). Therefore, the size of uface is (nz - 1, ny - 1, nx), the
    # size of vface is (nz - 1, ny, nx - 1) and the size of wface is (nz, ny - 1, nx - 1)
    uface = ds.u[time_index, ...]
    vface = ds.v[time_index, ...]
    wface = ds.w[time_index, ...]

    # trajectories tendencies, ie the right-hand side of dx/dt = u
    def vel_fun(t, pos):

        # get all the x, y, z coordinates. DO WE NEED TO COPY?
        xi = pos[0*n:1*n].copy()
        yi = pos[1*n:2*n].copy()
        zi = pos[2*n:3*n].copy()

        # find the cells containing the xi, yi and zi points
        ...
        # compute the parametric coordinates of the cell (0 <= xsi, eta, zet <= 1)
        zet = kfloat - k0
        eta = jfloat - j0
        xsi = ifloat - i0
        tez = 1. - zet
        ...
        # interpolate the velocity at the xi, yi, zi points. Note: we assume that u is piecewise
        # constant in y and z, component v is piecewise contant in x and z and component w is 
        # constant in x and y. The u, v, w components are linearly interpolated in the directions 
        ...
        # return the velocities at xi. yi, zi
    ...
    #
    # compute the elements deformation gradient F matrix
    #
